describe('homepage', () => {
  ...
  it('should create a new trick when the input fields are filled', () => {
    cy.get('[name="stance"]').select('switch')
    cy.get('.trick-name').type('tricky trick')
    cy.get('[name="obstacle"]').select('Rail')
    cy.get('.tutorial-link').type('http://yadayada')
    cy.get('.send-btn').click()
    cy.get('.tricks-cont').children().should('have.length', 4)
    cy.get('.card').first().contains('p', 'switch treflip')
    cy.get('.card').first().contains('p', 'Obstacle: flat ground')
    cy.get('.card').first().contains('p', 'Link to Tutorial')
    cy.get('.card').first().contains('p', 'https://www.youtube.com/watch?v=XGw3YkQmNig')
    cy.get('.card').last().contains('p', 'switch tricky trick')
    cy.get('.card').last().contains('p', 'Obstacle: Rail')
    cy.get('.card').last().contains('p', 'Link to Tutorial')
    cy.get('.card').last().contains('p', 'http://yadayada')
  })
})